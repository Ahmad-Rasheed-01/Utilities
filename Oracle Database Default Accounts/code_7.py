...
class OracleAccountComparator(QMainWindow):
    def __init__(self):
        ...
        # Initialize variables to store comparison results
        self.matched_accounts = set()
        self.unmatched_accounts = set()
    ...
    def analyze_accounts(self):
        try:
            ...
            # Update instance variables with the current analysis
            self.matched_accounts = set(user_accounts) & set(self.default_accounts)
            self.unmatched_accounts = set(user_accounts) - set(self.default_accounts)

            self.result_browser.append("\n***Comparison Results***")
            self.result_browser.append(f"\nMatched Accounts: {', '.join(self.matched_accounts)}")
            self.result_browser.append(f"\nUnmatched Accounts: {', '.join(self.unmatched_accounts)}")
